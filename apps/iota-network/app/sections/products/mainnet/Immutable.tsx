...
import { IMMUTABLE_CTA_CARD_CONTENT } from './constants';

export function Immutable({ id, navbarColorScheme }: BaseSectionProps) {
    return (
        <section className="w-full" id={id} data-navbar-color-scheme={navbarColorScheme}>
            <div className="container py-14 sm:py-20 lg:py-30 flex justify-center items-center w-full">
                <div className="grid grid-cols-1 xs:grid-cols-2 gap-6">
                    {IMMUTABLE_CTA_CARD_CONTENT.map((data, index) => (
                        <CtaCard
                            title={data.title}
